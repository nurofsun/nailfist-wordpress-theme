...
if ( ! function_exists( 'nailfist_posted_on' ) ) :
	...
	function nailfist_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'nailfist' ),
...
if ( ! function_exists( 'nailfist_posted_by' ) ) :
	...
	function nailfist_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'nailfist' ),
...
if ( ! function_exists( 'nailfist_entry_footer' ) ) :
	...
	function nailfist_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'nailfist' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'nailfist' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'nailfist' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
